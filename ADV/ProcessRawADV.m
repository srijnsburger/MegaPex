...
adv=load('G355001-hd1.mat'); %this contains the time of each adv burst
time_adv=adv.Day;
...
eval(['load(''ADVG',num2str(k),'_v1.mat'')']);
vx=v1;
eval(['load(''ADVG',num2str(k),'_v2.mat'')']);
vy=v2;
eval(['load(''ADVG',num2str(k),'_v3.mat'')']);
vz=v3;
...
set(gcf,'Color','w')
s1=subplot(311);
eval(['plot(adv',num2str(k),'.time(1:end-1),adv',num2str(k),'.east_vel)'])
hold on
plot(time(2000000:3250000),-vel(2000000:3250000,1),'g')
s2=subplot(312);
eval(['plot(adv',num2str(k),'.time(1:end-1),adv',num2str(k),'.north_vel)'])
hold on
plot(time(2000000:3250000),vel(2000000:3250000,2),'g')
s3=subplot(313);
eval(['plot(adv',num2str(k),'.time(1:end-1),adv',num2str(k),'.up)'])
hold on
plot(time(2000000:3250000),vel(2000000:3250000,3),'g')
linkaxes([s1 s2 s3],'x')


eval(['adv',num2str(k),'.raw.ee=-vel(:,1)']);
eval(['adv',num2str(k),'.raw.nn=vel(:,2)']);
eval(['adv',num2str(k),'.raw.uu=vel(:,3)']);
